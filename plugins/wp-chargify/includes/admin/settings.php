 ...
 *
 * @file wp-chargify/includes/admin/settings.php
 * @package WPChargify
 */
...
function register_chargify_options_metabox() {
	...
	$products_options->add_field(
		[
			...
			'desc' => __( 'Select the Chargify Products you want to import below. This will also import their Price Points.', 'chargify' ),
			'type' => 'title',
	...
	// Allow developers to hide the Component options tab.
	$hide_component_options = apply_filters( 'chargify_hide_components_options', false );

	if ( ! $hide_component_options ) {
		/**
		 * Registers the Components options.
		 */
		$args = [
			'id'           => 'component_page',
			'title'        => __( 'component', 'chargify' ),
			'object_types' => [ 'options-page' ],
			'option_key'   => 'chargify_component',
			'parent_slug'  => 'chargify_options',
			'tab_group'    => 'chargify_options',
			'tab_title'    => __( 'Components', 'chargify' ),
		];

		$components_options = new_cmb2_box( $args );

		$components_options->add_field(
			[
				'name' => __( 'Chargify Components', 'chargify' ),
				'desc' => __( 'Select the Chargify Components you want to import below. This will also import their Price Points.', 'chargify' ),
				'type' => 'title',
				'id'   => 'chargify_components_title',
			]
		);
		...
		$components_options->add_field(
			[
				'name'       => __( 'Chargify Components', 'chargify' ),
				'desc'       => __( 'Select the Chargify Components you\'d like to use in WordPress', 'chargify' ),
				'id'         => 'chargify_components_multicheck',
				'type'       => 'multicheck',
				'options_cb' => 'Chargify\\Post_Types\\Helpers\\get_product_values',
			]
		);
		...
		$components_options->add_field(
			[
				'name' => __( 'Resync Components', 'chargify' ),
				'desc' => __( 'Select this checkbox and click "Save Changes" to trigger a resync of your Chargify components.' ),
				'id'   => 'chargify_resync_chargify',
				'type' => 'checkbox',
			]
		);
